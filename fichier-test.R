...
if(n>=30){
  cat(paste("comme n>=30 Alors on vas utiliser la loi normale, Test Bilateral\n"))
  cat("\n")
  s_xbar = s/sqrt(n)
  cat(paste(s_xbar))
  ...
  cat("\n")
  cat(paste("Zone de non rejet de H0 est :[",borne_inf,";",borne_sup,"] \n"))
  ...
  if(z_obs>=borne_inf & z_obs<=borne_sup){
    cat(paste(z_obs,"appartient inclus a [",borne_inf,";",borne_sup,"] \n"))
    cat("\n")
    cat(paste("Ho est accepte \n"))
  }else{
    cat(paste(z_obs,"appartient non inclus dans [",borne_inf,";",borne_sup,"] \n"))
    cat("\n")
    cat(paste("Donc H0 est rejette et H1 est accepte \n"))
    cat("\n")
    cat(paste("Conclusion \n"))
    cat("\n")
    cat(paste("Au risque de se tromper  de ",seuil_signifi*100,"% nous pouvons remettre en doute l'affirmation du chef de departement\n"))
    
    x_val<-seq(-4,4,length=1000)
    y_val<-dnorm(x_val)
    plot(x_val,y_val,type="l",lwd=2,col="blue",main="Loi Normale - Test Bilateral",xlab="Z",ylab="Densite")
    ...
    x_mid<-seq(borne_inf,borne_sup,length=200)
    polygon(c(borne_inf,x_mid,borne_sup),c(0,dnorm(x_mid),0),col="lightgreen",border = NA)
